...
module.exports = {
  ...
  getCharacters: async (req, res) => {
    try {
      const posts = await Post.find({ user: req.user.id });
      res.render("my-characters.ejs", { posts: posts, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  getBookmarks: async (req, res) => {
    try {
      const posts = await Post.find({ user: req.user.id });
      res.render("saved-characters.ejs", { posts: posts, user: req.user });
    } catch (err) {
      console.log(err);
    }
  },
  ...
  createPost: async (req, res) => {
    try {
      ...
      await Post.create({
        ...
        chDescription: req.body.chDescription.trim().split('\n'),
        addColors: req.body.addColors.trim().split('\n'),
        likes: 0,
        user: req.user.id,
  ...
  likePost: async (req, res)=>{
    ...
    //if already liked we'll remove user from likes array
    if(liked){
      try{
    ...
    bookmarkPost: async (req, res)=>{
      ...
      //if already bookmarked we'll remove user from likes array
      if(bookmarked){
        try{
          await Post.findOneAndUpdate({_id:req.params.id},
            {
